...
export const CountNavigate = (props: IBooksCategories) => {
  ...
  return (
    <NavigateBooksCount data-test-id={`navigation-book-count-for-${props.path}`}>{booksInfo.filter((book) => book.categories.includes(props.name)).length}</NavigateBooksCount>
  );
};